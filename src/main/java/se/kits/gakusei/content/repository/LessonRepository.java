...
public interface LessonRepository
    ...
    @Cacheable("lessons.numbers")
    Integer findNumberOfKanjisByName(
            @Param("lessonName") String lessonName
    );
    ...
    @Query(value = "SELECT COUNT(id) FROM contentschema.kanjis WHERE id IN\n" +
            ...
            nativeQuery = true)
    Integer findNumberOfCorrectlyAnsweredKanjis(
            @Param("username")
                    String username,
            @Param("lessonName")
                    String lessonName
    ...
    @Query(value = "SELECT COUNT(contentschema.kanjis.id) FROM contentschema.kanjis\n" +
            ...
            "AND lesson_id IN (SELECT id FROM contentschema.lessons WHERE name = :lessonName) ORDER BY retention_date ASC)",
            nativeQuery = true)
    Integer findNumberOfKanjisByRetentionDate(
            @Param("username")
                    String username,
            @Param("lessonName")
                    String lessonName
    ...
    @Query(value = "SELECT COUNT(contentschema.kanjis.id) FROM contentschema.kanjis\n" +
            ...
            nativeQuery = true)
    Integer findNumberOfUnansweredRetentionKanjis(
            @Param("username")
                    String username,
            @Param("lessonName")
                    String lessonName

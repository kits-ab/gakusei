...
public class DataInit implements ApplicationRunner {
    ...
    public void run(ApplicationArguments applicationArguments)
        ...
        if (datainit) {
            ...
            allKeysExceptBooks = new HashSet<>(
                Arrays.asList(
                    "type",
                    "english",
                    "swedish",
            ...
            logger.info(
                "*** Data initialization was NOT set on profile(s): " + activeProfiles
            );
        }
    }
    ...
    private void createTestData(Set<Map<String, Object>> dataHolders) {
        for (Map<String, Object> tdh : dataHolders) {
            try {
                String nuggetType = ((ArrayList<String>) tdh.get("type")).get(
                    0
                );
                ...
                for (Map.Entry entry : tdh.entrySet()) {
                    ...
                    if (!allKeysExceptBooks.contains(type)) {
                        Book book = bookRepository.findByTitle(
                            getBookTitle(type, entry.getValue())
                        );
    ...
    private void createUsers() {
        ...
        users.add(
            new User("nulluser", passwordEncoder.encode("gakusei"), "NULL_USER")
        );
        users.add(
            new User("pieru", passwordEncoder.encode("gakusei"), "ROLE_USER")
        );
        users.add(
            new User("yoakimu", passwordEncoder.encode("gakusei"), "ROLE_USER")
        );
        users.add(
            new User("pa", passwordEncoder.encode("gakusei"), "ROLE_USER")
        );
        users.add(
            new User("debiddo", passwordEncoder.encode("gakusei"), "ROLE_USER")
        );
        users.add(
    ...
    private void createVerbLesson() {
        List<Nugget> verbNuggets = nuggetRepository.findAll().stream().filter(
            n -> n.getWordType().getType().equals("verb")
    ...
    private Lesson createLesson(Book book) {
        List<Nugget> nuggets = nuggetRepository.findAll().stream().filter(
            n -> n.getBooks().stream().map(Book::getId).collect(
                Collectors.toList()
            ).contains(book.getId())
        ).collect(Collectors.toList());
        List<Kanji> kanjis = StreamSupport.stream(
            kanjiRepository.findAll().spliterator(),
            false
        ).filter(
            k -> k.getBooks().stream().map(Book::getId).collect(
                Collectors.toList()
            ).contains(book.getId())
        ).collect(Collectors.toList());
        Lesson lesson = new Lesson();
    ...
    public void createQuizzesFromCSV(String csvFile) throws Exception {
        CSVQuizToDatabase parser = new CSVQuizToDatabase(
            resourceLoader.getResource("classpath:" + csvFile).getInputStream()
        );

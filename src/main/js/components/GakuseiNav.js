...
export default class GakuseiNav extends React.Component {
  eventHandler(eventKey) {
    if (eventKey === 0) {
      ...
      this.props.updater('LessonSelection', { gamemode: 'GuessPlayPage' });
    } else if (eventKey === 1.2) {
      this.props.updater('LessonSelection', { gamemode: 'TranslationPlayPage' });
    } else if (eventKey === 2) {
  ...
  render() {
    return (
      <Navbar onSelect={this.eventHandler.bind(this)} inverse collapseOnSelect>
        <Navbar.Header>
          <NavbarBrand>
            <span onClick={() => this.eventHandler(0)}>
              <a>
